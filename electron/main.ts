...
function handleChangeProtonApp(
  ...
  if (title === "calendar") {
    win?.loadURL("https://calendar.proton.me/");
  } else if (title === "email") {
    win?.loadURL("https://mail.proton.me");
  }
}

function goBackToHubPage(): void {
  if (url) {
    win?.loadURL(url);
  } else {
...
function setContextMenu(state: boolean | null = null) {
  if (state === null) state = win?.isVisible() ?? true;
  const visibilityLabel = (state ? "Hide" : "Show") + " ProtonHub";
  const contextMenu = Menu.buildFromTemplate([
    ...
    {
      label: "Go back to HubPage",
      type: "normal",
      commandId: 3,
      click: goBackToHubPage,
    },
    quitApp,
  ]);
...
function applicationMenu() {
  ...
  app!.applicationMenu!.insert(0, appMenu);
}
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    // change font in all app 
    win?.webContents.executeJavaScript(`
      document.body.style.font = '18px system-ui';
    `);
  });
...
app.whenReady().then(() => {
  ipcMain.on("change-app", handleChangeProtonApp);
  applicationMenu();
  createTrayMenu();
  createWindow();
});
